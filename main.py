...
class AED(torch.nn.Module):
    def __init__(self, I, H, L, num=1, heads=24):
        super(AED, self).__init__()
        self.encoder_layers = torch.nn.ModuleList()
        self.encoder_layers.append(NGATConv(I, H, heads=heads))
        for _ in range(num - 1):
            self.encoder_layers.append(NGATConv(H * heads, H, heads=heads))
        self.linear = torch.nn.Linear(H * heads, L)
        self.decoder_layers = torch.nn.ModuleList()
        self.decoder_layers.append(NGATConv(L, H, heads=heads))
        for _ in range(num - 2):
            self.decoder_layers.append(NGATConv(H * heads, H, heads=heads))
        self.decoder_layers.append(NGATConv(H * heads, I, heads=1))
...
class DTIV7(nn.Sequential):
    def __init__(self, DAED, TAED,  **params):
        super(DTIV7, self).__init__()
        self.DAED = DAED
        self.TAED = TAED
        self.Dset2set = Set2Set(128, processing_steps=3)
        self.Tset2set = Set2Set(128, processing_steps=3)
    ...
    def forward(self, DV, TV, D3D, T3D):
        A = self.Dset2set(self.DAED(D3D), D3D.batch)
        B = self.Tset2set(self.TAED(T3D), T3D.batch)
        # print(B.shape)
        # quit()
        A = torch.unsqueeze(A.unsqueeze(1) , 2).repeat(1, self.Gsize, self.Gsize, 1)
        B = torch.unsqueeze(B.unsqueeze(1), 1).repeat(1, self.Gsize, self.Gsize, 1)
        AB = A * B
...
def start():
    ...
    if params['Kfold']:
        ...
        for i in range(params['fold']):
            ...
            model = DTIV7(DAED, TAED, **params).to(device)
